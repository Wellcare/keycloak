...
public class WellcareSmsSenderServiceProvider extends FullSmsSenderAbstractService {
    ...
    private static final Logger logger = Logger.getLogger(WellcareSmsSenderServiceProvider.class);
    ...
    private static class WellcareMessage {
        public String from;
        public String to;
        public String body;
        ...
        public WellcareMessage(String from, String to, String body, String encoding, String routingGroup) {
            this.from = from;
            this.to = to;
            this.body = body;
    ...
    WellcareSmsSenderServiceProvider(Scope config, KeycloakSession session) {
        super(session);

        String configUrl = config.get(CONFIG_API_SERVER);
        this.url = configUrl != null ? configUrl : "https://wellcare.vn";
    ...
    public void sendMessage(String phoneNumber, String message) throws MessageSendException {
        ...
        req.json(new WellcareMessage[] {
                new WellcareMessage(this.from, phoneNumber, message, this.encoding, this.routingGroup) });
        ...
        try {
            ...
            if (res.getStatus() >= 200 || res.getStatus() <= 299) {
                ...
                throw new MessageSendException("Wellcare API responded with an error.", new Exception(res.asString()));
            }
            ...
            throw new MessageSendException("Error while communicating with Wellcare API.", ex);
        }
    }
