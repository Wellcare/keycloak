...
public class DefaultPhoneProvider implements PhoneProvider {
    ...
    private final int targetDayMaximum;
    ...
    DefaultPhoneProvider(KeycloakSession session, Scope config) {
        ...
        this.targetDayMaximum = config.getInt("targetDayMaximum",3);
    ...
    public int sendTokenCode(String phoneNumber,String sourceAddr,TokenCodeType type, String kind){
        ...
        if (getTokenCodeService().isAbusing(phoneNumber, type,sourceAddr, sourceHourMaximum, targetHourMaximum, targetDayMaximum)) {
            throw new ForbiddenException("You requested the maximum number of messages the day");
        }
